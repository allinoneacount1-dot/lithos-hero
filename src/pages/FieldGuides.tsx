...
function StatItem({ s, i }: { s: typeof guideStats[number]; i: number }) {
...
export default function FieldGuides() {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] pt-28 px-6 sm:px-14 relative">
      ...
      <div className="absolute top-20 left-1/3 w-[500px] h-[500px] bg-white/3 blur-[150px] rounded-full pointer-events-none hero-glow-orbit" />
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-10">
          {guideStats.map((s, i) => <StatItem key={i} s={s} i={i} />)}
        </div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-16">
          {filtered.map((g, i) => <GuideCard key={i} g={g} i={i} />)}
        </div>

        {/* Featured Guide Banner */}
        <div ref={featuredRef} className="hero-scroll-reveal" style={{ transitionDelay: '0.15s' }}>
          <div className="bg-gradient-to-r from-white/5 to-transparent border border-white/10 rounded-xl p-6 sm:p-8 mb-10">
