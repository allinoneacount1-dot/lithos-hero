...
function GeoStatItem({ s, i }: { s: typeof geoStats[number]; i: number }) {
...
function TopicCard({ t, i }: { t: typeof topics[number]; i: number }) {
  const ref = useScrollReveal<HTMLDivElement>()
  ...
  return (
    <div ref={ref} className="hero-scroll-reveal" style={{ transitionDelay: `${i * 0.07}s` }}>
      <div
        ...
        className="hero-card group bg-white/5 border border-white/10 rounded-xl p-5 relative overflow-hidden cursor-pointer"
      >
...
export default function Geology() {
  ...
  const quickRef = useScrollReveal<HTMLDivElement>()
  const newsletterRef = useScrollReveal<HTMLDivElement>()

  return (
    <div className="min-h-screen bg-[#0a0a0a] pt-28 px-6 sm:px-14 relative">
      <div className="hero-grain" />
      <div className="absolute top-40 right-10 w-[450px] h-[450px] bg-[#e8702a]/3 blur-[140px] rounded-full pointer-events-none hero-glow-orbit-reverse" />
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-10">
          {geoStats.map((s, i) => <GeoStatItem key={i} s={s} i={i} />)}
        </div>

        {/* Quick Links */}
        <div ref={quickRef} className="hero-scroll-reveal flex flex-wrap gap-2 mb-10" style={{ transitionDelay: '0.1s' }}>
          {quickLinks.map((l, i) => {
            ...
            return (
              ...
              >
                <LinkIcon size={12} className="text-white/40 group-hover:text-[#e8702a]" />
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-16">
          {topics.map((t, i) => <TopicCard key={i} t={t} i={i} />)}
        </div>

        {/* Newsletter */}
        <div ref={newsletterRef} className="hero-scroll-reveal" style={{ transitionDelay: '0.2s' }}>
